...
class DogsAPI {
    ...
    func getNextDogPublisher(_ idx: Int) -> AnyPublisher<DogPayload, Never>{
        Future<DogPayload, Never> { promise in
            DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1)) {
                print("Query received by fake server API")
                let id = (idx+1) < self.dogs.count ? (idx+1) : 0
                let dog = self.dogs[id]
                let payload = DogPayload(id: id, dog: dog)
